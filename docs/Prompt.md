...
- *.s 泛域名 本地已被 dnsmasq 解析到 127.0.0.1，后期泛域名和地址都可以通过启动参数配置
...
主要功能：
- 代理服务运行在 admin.s:<port> 下
- 当访问 phx-demo.s:<port>时，查看对应应用是否运行；如果没有运行，根据应用配置，自动拉启
...
- server 配置添加 允许的后缀 列表，默认为： .l (for http), .s (for https)
...
有个设计权衡，现实中不太可能出现demo.l和demo.s，因为注册时就会发现有同名应用了，通过不同的name解决类似需求。之前考虑过使用域名唯一解决，感觉方案有点复杂，而且域名本身可通过域名解析层重定向，这里只处理底层应用启动需求
